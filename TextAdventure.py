...
class Map:
    ...
    def forward(self):
        if self.x == len(self.state) - 1:
            print("Du siehst ein riesiges Gebirge was du nicht passieren kannst")
        else:
            self.x = self.x + 1

    def backwards(self):
        if self.x == 0:
            print("Du Siehst Klippen, aber kannst nicht sicher runterspringen")
        else:
            self.x = self.x - 1
...
def quit_game(p, m):
    print("Du begehst Selbstmord und verlässt diese Welt.")
    exit(0)
...
def fight(p, m):
    ...
    while len(enemies) > 0:
        ...
        for i in enemies:
            p.get_hit(i.ad)
        print("Du wurdest verwundet " + str(p.hp) + " hp übrig")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if command[0] in Commands:
            ...
            print("Du gehst im Kreis und weißt nicht was du tun sollst.")
        map.print_state()